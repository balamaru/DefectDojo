...
- Top 10 agent detail CVE (full description tanpa potongan)
- Action Required untuk setiap temuan
- DDoS section dengan action items
...
def determine_action_required(cve: dict) -> str:
    """Determine action required based on severity, CVSS, and package criticality."""
    severity = cve.get("severity", "Medium")
    cvss = cve.get("cvss", 0) or 0
    package = cve.get("package_name", "")
    
    critical_packages = ["kernel", "openssl", "glibc", "systemd", "docker", "kubernetes", "libc", "nginx", "apache"]
    
    if severity == "Critical" or cvss >= 9.0:
        return "IMMEDIATE: Patch within 24 hours"
        ...
        return "SCHEDULED: Patch within 7 days"
    elif severity == "Medium" or cvss >= 4.0:
        return "PLANNED: Patch within 30 days"
    else:
        return "REVIEW: Assess impact"


def determine_agent_action(agent: dict) -> tuple:
    """Return (action_text, action_color) for an agent."""
    if agent.get("critical_cve_count", 0) > 0:
        return "IMMEDIATE PATCH REQUIRED", C_CRITICAL
    elif agent.get("high_cve_count", 0) > 0:
        return "SCHEDULED PATCH (7 days)", C_HIGH
    elif agent.get("medium_cve_count", 0) > 0:
        return "PLANNED UPDATE (30 days)", C_MEDIUM
    else:
        return "MONITOR ONLY", C_LOW


def determine_ddos_action(alert_count: int, confidence: str) -> tuple:
    """Return (action_text, attack_type, action_color) for DDoS."""
    if confidence == "High":
        return "BLOCK: Confirmed Malicious Source", "Verified Attack Source", C_CRITICAL
    elif alert_count > 100:
        return "IMMEDIATE: Block IP / Rate Limit", "Potential DDoS Flood", C_CRITICAL
    elif alert_count > 50:
        return "URGENT: Investigate & Consider Block", "Aggressive Scanning", C_HIGH
    elif alert_count > 20:
        return "SCHEDULED: Deep Packet Inspection", "Suspicious Activity", C_MEDIUM
    elif confidence == "Medium":
        return "MONITOR: Add to Watchlist", "Suspicious Source", C_MEDIUM
    else:
        return "REVIEW: Log Analysis", "Low-level Probing", C_LOW
...
def build_executive_summary(story, styles, canonical):
    ...
    findings_data = [
        ...
        ["DefectDojo Import Status",
         str(dojo.get("import_status","pending")).upper(),
...
def build_all_agents_section(story, styles, canonical):
    ...
    for sev in severities:
        ...
        header = ["#", "Agent ID", "Agent Name", "OS", "Critical", "High", "Medium", "Max CVSS", "Action Recommended"]
        rows   = [header]
        
        for idx, a in enumerate(agents_in_group, 1):
            action, action_color = determine_agent_action(a)
            rows.append([
                str(idx),
                str(a.get("agent_id", "")),
                ...
                Paragraph(f'<font color="{SEV_HEX_MAP.get(action_color, "#000000")}">{action}</font>', 
                          styles["body_small"]),
            ])

        col_w = [10*mm, 18*mm, 40*mm, 45*mm, 16*mm, 16*mm, 16*mm, 18*mm, 35*mm]
...
def build_top_agents_detail(story, styles, canonical):
    ...
    for rank, agent in enumerate(top_agents, 1):
        ...
        header = ["CVE ID", "Sev", "CVSS", "Package", "Version", "Description", "Action Required"]
        rows   = [header]
        
        for cve in cves:
            desc = str(cve.get("description") or "")
            action = determine_action_required(cve)
            
            rows.append([
                str(cve.get("cve_id", "")),
                str(cve.get("severity", "")),
                str(cve.get("cvss", "N/A")),
                str(cve.get("package_name", ""))[:25],
                str(cve.get("package_version", ""))[:18],
        ...
        col_w = [28*mm, 16*mm, 14*mm, 28*mm, 20*mm, 50*mm, 35*mm]
        tbl   = Table(rows, colWidths=col_w, repeatRows=1)
        ...
        for i, row in enumerate(rows[1:], 1):
            ...
            action_str = row[6] if isinstance(row[6], str) else ""
            if "IMMEDIATE" in action_str.upper():
                ts.add("TEXTCOLOR", (6,i), (6,i), C_CRITICAL)
                ts.add("FONTNAME",  (6,i), (6,i), "Helvetica-Bold")
            elif "URGENT" in action_str.upper():
                ts.add("TEXTCOLOR", (6,i), (6,i), C_HIGH)

        # Now wrap action strings as Paragraphs for proper text rendering in cells
        for row in rows[1:]:
            if isinstance(row[6], str):
                row[6] = Paragraph(row[6], styles["body_small"])
        ...
        #     action_sev = row[6]
        #     action_text = action_sev if isinstance(action_sev, str) else str(action_sev)
        #     if "IMMEDIATE" in action_sev.upper():
        #         ts.add("TEXTCOLOR", (6,i), (6,i), C_CRITICAL)
        #         ts.add("FONTNAME",  (6,i), (6,i), "Helvetica-Bold")
        #     elif "URGENT" in action_sev.upper():
        #         ts.add("TEXTCOLOR", (6,i), (6,i), C_HIGH)
...
def build_cvss_analysis(story, styles, canonical):
    ...
    header = ["Rank", "Agent Name", "Max CVSS", "Avg CVSS", "Risk Level", "Action Required"]
    rows   = [header]
    ...
    for i, a in enumerate(top_agents, 1):
        ...
        if max_c and float(max_c) >= 9.0:
            ...
            risk = "HIGH"
            action = "SCHEDULED PATCH (7 days)"
        elif max_c and float(max_c) >= 4.0:
            risk = "MEDIUM"
            action = "PLANNED UPDATE (30 days)"
        else:
            risk = "LOW"
            action = "MONITOR"
    ...
    col_w = [15*mm, 55*mm, 25*mm, 25*mm, 25*mm, 40*mm]
    tbl   = Table(rows, colWidths=col_w, repeatRows=1)
...
def build_ddos_section(story, styles, canonical):
    ...
    total_alerts = sum(item.get("alert_count", 0) for item in candidates)
    unique_ips = len(candidates)
    
    ddos_kpi = [
        ["Total DDoS Alerts", str(total_alerts)],
        ["Unique Source IPs", str(unique_ips)],
        ["High Confidence IPs", str(len(malicious.get("high_confidence", [])))],
        ["Under Investigation", str(len(malicious.get("medium_confidence", [])))],
    ]
    
    kpi_tbl = Table(ddos_kpi, colWidths=[50*mm, 40*mm])
    ...
    story.append(Paragraph("DDoS Attack Details & Required Actions", styles["h2"]))
    
    header = ["#", "Source IP", "Alert Count", "TI Status", "Attack Type", "Action Required"]
    rows   = [header]
    
    for i, item in enumerate(candidates, 1):
        ip = item.get("ip", "")
        alert_count = item.get("alert_count", 0)
        confidence = item.get("confidence", "Pending")
        
        action, attack_type, action_color = determine_ddos_action(alert_count, confidence)
    ...
    col_w = [12*mm, 45*mm, 25*mm, 30*mm, 40*mm, 45*mm]
    tbl   = Table(rows, colWidths=col_w, repeatRows=1)
    ts    = _base_table_style(C_ACCENT2)
    
    for i, row in enumerate(rows[1:], 1):
        action = row[5]
        if "BLOCK" in action.upper() or "IMMEDIATE" in action.upper():
            ts.add("BACKGROUND", (0,i), (-1,i), colors.HexColor("#FDEDEC"))
            ts.add("TEXTCOLOR", (5,i), (5,i), C_CRITICAL)
            ts.add("FONTNAME",  (5,i), (5,i), "Helvetica-Bold")
        elif "URGENT" in action.upper():
            ts.add("BACKGROUND", (0,i), (-1,i), colors.HexColor("#FEF9E7"))
            ts.add("TEXTCOLOR", (5,i), (5,i), C_HIGH)
...
def build_malicious_ip_section(story, styles, canonical):
    ...
    if high_conf:
        header = ["IP Address", "Alerts", "VT Malicious", "OTX Pulses",
                  "AbuseIPDB Score", "Total Reports", "Action Required"]
        rows   = [header]
    ...
    if med_conf:
        header = ["IP Address", "Alerts", "VT Malicious", "OTX Pulses",
                  "AbuseIPDB Score", "Action Required"]
        rows   = [header]
...
def build_dojo_summary(story, styles, canonical):
    ...
    data = [
        ...
        ["Import Status",               str(dojo.get("import_status","pending")).upper()],
...
def fetch_ddos_alerts(period: dict, top_n: int = 100) -> list:
    """Fetch detailed DDoS alerts with attack classification."""
    body = {
        "size": top_n,
        ...
        "query": {"bool": {"filter": [
            ...
            {"terms": {"rule.groups": ["attack", "web", "accesslog", "ddos"]}},
            {"exists": {"field": "data.srcip"}}
        ]}},
        "_source": ["@timestamp", "rule.id", "rule.description", "data.srcip", 
                    "data.protocol", "full_log", "agent.name"]
    }
    try:
        data = indexer_post("/wazuh-alerts*/_search", body)
        ...
        print(f"[WARN] fetch_ddos_alerts failed: {e}")
        return []
...
def build_canonical_dataset(payload: dict) -> dict:
    ...
    print("[INFO] Fetching detailed DDoS alerts...")
    ddos_alerts = fetch_ddos_alerts(period, top_n=100)
